...
if (EW_this.x_ser_Prefijo && !EW_hasValue(EW_this.x_ser_Prefijo, "TEXT" )) {
	if (!EW_onError(EW_this, EW_this.x_ser_Prefijo, "TEXT", "Favor de agregar - Prefijo"))
		return false;
}
return true;
}
...
Prefijo
<?php }else{ ?>
	<a href="series_listado.php?order=<?php echo urlencode("ser_Prefijo"); ?>">Prefijo&nbsp;(*)<?php if (@$_SESSION["vit_series_x_ser_Prefijo_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["vit_series_x_ser_Prefijo_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</span></th>
		<th><span>
<?php if ($sExport <> "") { ?>
...
<input class="form-control" type="text" name="x_ser_Prefijo" id="x_ser_Prefijo" size="30" maxlength="60" value="<?php echo htmlspecialchars(@$x_ser_Prefijo) ?>">
<?php }else{ ?>
...
$x_ser_EjercicioList .= "<option value=''>Please Select</option>";
...
<?php } ?>
		...
		<td><span class="phpmaker">
<?php if ($bEditRow) { // Edit Record ?>
<input class="form-control" type="text" name="x_ser_Numero" id="x_ser_Numero" size="30" maxlength="9" value="<?php echo htmlspecialchars(@$x_ser_Numero) ?>">
<?php }else{ ?>
...
<?php } ?>
		...
		<td><span class="phpmaker">
<?php if ($bEditRow) { // Edit Record ?>
<input class="form-control" type="text" name="x_ser_Serie" id="x_ser_Serie" size="30" maxlength="150" value="<?php echo htmlspecialchars(@$x_ser_Serie) ?>" readonly>
<?php }else{ ?>
<?php echo $x_ser_Serie; ?>
<?php } ?>
</span></td>
<?php if ($sExport == "") { ?>
<!--<td><span class="phpmaker"><a href="<?php if ($x_ser_SerieID <> "") {echo "vit_seriesview.php?ser_SerieID=" . urlencode($x_ser_SerieID); } else { echo "javascript:alert('Invalid Record! Key is null');";} ?>">View</a></span></td>-->
<td><span class="phpmaker">
...
<!--<td><span class="phpmaker"><a href="<?php if ($x_ser_SerieID <> "") {echo "series_listado.php?ser_SerieID=" . urlencode($x_ser_SerieID); } else { echo "javascript:alert('Invalid Record! Key is null');";} ?>">Edit</a></span></td>
<td><span class="phpmaker"><a href="<?php if ($x_ser_SerieID <> "") {echo "vit_seriesdelete.php?ser_SerieID=" . urlencode($x_ser_SerieID); } else { echo "javascript:alert('Invalid Record! Key is null');";} ?>">Delete</a></span></td>-->
<?php } ?>
	</tr>
<?php
	}
...
if (EW_this.n_ser_Prefijo && !EW_hasValue(EW_this.n_ser_Prefijo, "TEXT" )) {
	if (!EW_onError(EW_this, EW_this.n_ser_Prefijo, "TEXT", "Favor de ingresar - Prefijo"))
		return false;
}
return true;
}
                                        ...
                                        <!--<form action="" class="d-flex flex-column justify-content-end h-100">-->
                                            <div class="offcanvas-body">
												<div class="mb-4">
													...
													<?php
													$x_Rec_TipoJornadaList = "<select name=\"x_Emisor_ID\" class=\"form-select form-select\">";
													$x_Rec_TipoJornadaList .= "<option value=''>Favor de elegir</option>";
													$sSqlWrk = "SELECT DISTINCT `Emi_Clave`, `Emi_NomCorto` FROM `vit_emisor`";
													...
													if ($rswrk) {
														...
														while ($datawrk = phpmkr_fetch_array($rswrk)) {
															...
															$x_Rec_TipoJornadaList .= ">" . $datawrk["Emi_Clave"] . " - " . $datawrk["Emi_NomCorto"] . "</option>";
															$rowcntwrk++;
														}
													}
