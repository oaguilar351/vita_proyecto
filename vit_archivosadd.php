...
function AddData($conn)
{
	...
	if (!empty($_FILES["n_arch_Ruta"]["size"])) {
		if (!empty($EW_MaxFileSize) && $_FILES["n_arch_Ruta"]["size"] > $EW_MaxFileSize) {
			die("Max. file upload size exceeded");
		}
	}

	// Field arch_Ruta
		if (is_uploaded_file($_FILES["n_arch_Ruta"]["tmp_name"])) {
			$destfile = ewUploadPath(1) . ewUploadFileName($_FILES["n_arch_Ruta"]["name"]);
					if (!move_uploaded_file($_FILES["n_arch_Ruta"]["tmp_name"], $destfile)) // move file to destination path
					die("You didn't upload a file or the file couldn't be moved to" . $destfile);

			// File Name
			$theName = (!(function_exists("get_magic_quotes_gpc") && (function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc()))) ? addslashes(ewUploadFileName($_FILES["n_arch_Ruta"]["name"])) : ewUploadFileName($_FILES["n_arch_Ruta"]["name"]);
			$fieldList["`arch_Ruta`"] = " '" . $theName . "'";
			@unlink($_FILES["n_arch_Ruta"]["tmp_name"]);
		}
	...
	if ($destfile!="" && $archivoID!="") {
		$reglon = 0;
		if (($gestor = fopen($destfile, "r")) !== FALSE) {
			while (($datos = fgetcsv($gestor, 1000, ",")) !== FALSE) {
				$numero = count($datos);
				$reglon++;
				// Field arch_ArchivoID
				$theValue2 = ($archivoID != "") ? intval($archivoID) : "NULL";
				$fieldList2["`arch_ArchivoID`"] = $theValue2;
				
				// Field det_Renglon
				$theValue2 = ($reglon != "") ? " '" .intval($reglon) . "'" : "NULL";
				$fieldList2["`det_Renglon`"] = $theValue2;
				echo "<br />Columnas: ".$numero;
				for ($c=0; $c < $numero; $c++) {
						
					// Field det_Columna
					$theValue2 = ($c != "") ? " '" .($c) . "'" : "NULL";
					$fieldList2["`det_Columna`"] = $theValue2;
					
					//$datos2 = str_replace("$", "", $datos[$c]);
					///$datos2 = str_replace(",", "", $datos2);
					$datos2 = $datos[$c];
					
					// Field det_Valor
					$theValue2 = (!(function_exists("get_magic_quotes_gpc") && (function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc()))) ? ($datos2) : $datos2; 
					$theValue2 = ($theValue2 != "") ? " '" . $theValue2 . "'" : "NULL";
					$fieldList2["`det_Valor`"] = $theValue2;
					
					// Field det_FecReg
					$det_FecReg = date('Y-m-d H:i:s');
					$theValue = ($det_FecReg != "") ? " '" . ConvertDateToMysqlFormat($det_FecReg) . "'" : "NULL";
					$fieldList2["`det_FecReg`"] = $theValue;

					// Field det_UsuReg
					$det_UsuReg= @$_SESSION["project1_status_User"];
					$theValue = (!(function_exists("get_magic_quotes_gpc") && (function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc()))) ? addslashes($det_UsuReg) : $det_UsuReg; 
					$theValue = ($theValue != "") ? " '" . $theValue . "'" : "NULL";
					$fieldList2["`det_UsuReg`"] = $theValue;
					
					if(!(is_null($datos2)) && !(empty($datos2)) && (isset($datos2)) && $datos2!=' '){
						echo "<br />Columna: ".$c;
					// insert into database
					$sSql2 = "INSERT INTO `vit_archivos_detalle` (";
					$sSql2 .= implode(",", array_keys($fieldList2));
					$sSql2 .= ") VALUES (";
					$sSql2 .= implode(",", array_values($fieldList2));
					$sSql2 .= ")";
					echo "<br />sSql2: ".$sSql2;
					#phpmkr_query($sSql2, $conn) or die("Failed to execute query: " . phpmkr_error($conn) . '<br>SQL: ' . $sSql2);
					}
				}				
			}	
		}
